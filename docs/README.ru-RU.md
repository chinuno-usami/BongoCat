...
## 🔁 Синхронизация Steam по локальной сети (macOS → Windows)

Этот форк добавляет необязательный мост по локальной сети, который
зеркалирует локальные нажатия клавиш и клики мыши на компьютер с Windows,
где запущена сборка BongoCat для **Steam**. Кот из Steam затем шлёпает
лапами и накапливает прогресс PawPass / достижений от вашего ввода на Mac —
без ввода символов, перехвата фокуса или срабатывания сочетаний клавиш на
стороне Windows.

### Что синхронизируется

| Локальный ввод | Синхронизируется | Эффект на кота из Steam |
| --- | --- | --- |
| Нажатие клавиши клавиатуры | Да, как обычный тап | Шлепок лапой |
| Нажатие левой / правой кнопки мыши | Да, как обычный тап | Шлепок лапой |
| Перемещение мыши | Нет | Кот из Steam продолжает следовать за курсором Windows |
| Идентичность клавиши, позиция курсора | Нет | Не восстанавливаются приёмником |

Протокол намеренно основан на тапах: одна UDP-датаграмма `TAP:1` на порт
`39824` за каждое нажатие. Приёмнику достаточно добавить тап в счётчик
ввода игры, поэтому кот из Steam ведёт себя точно так же, как если бы
клавиша была нажата локально. Отправитель не зависит от рендерера Live2D,
поэтому он работает даже в сборках с диагностическим бэкендом.

### Отправитель для macOS (встроен в приложение)

Отправитель находится в `src/core/sync_net.c` и компилируется в приложение;
отдельный процесс не требуется. По умолчанию он включён в режиме
широковещания (`255.255.255.255:39824`). Цель определяется в следующем
порядке (последняя побеждает):

1. `sync.json` в рабочем каталоге
2. `~/Library/Application Support/BongoCat/config/sync.json`
3. Переменные окружения `BONGO_SYNC_IP` и `BONGO_SYNC_PORT`
4. Аргумент командной строки `--sync-ip <address>`
...
Указание конкретного `target_ip` переключает отправитель с широковещания на
прямой unicast, что рекомендуется, когда сеть блокирует широковещание UDP.
Сопутствующий набор `tools/` содержит шаблон `sync.json.example`.

### Приёмник для Windows (сборка Steam)

Приёмник поставляется в сопутствующем наборе `tools/`, прилагаемом к этому
форку. Доступны два варианта:

- **Вариант A — патч внутри игры (рекомендуется).** `install_steam_patch.bat`
  помещает `BongoSync.dll` в `BongoCat_Data\Managed\` и заменяет
  `Assembly-CSharp.dll` на пропатченную сборку, поэтому слушатель работает
  внутри процесса игры и больше ничего запускать не нужно.
  `uninstall_steam_patch.bat` восстанавливает исходные файлы.
- **Вариант B — автономный ретранслятор.** Запустите
  `win_bongo_receiver.py` на Windows. Он внедряет несимвольную виртуальную
  клавишу (`VK_NONAME`), которую сборка Steam уже отслеживает, не изменяя
  файлы игры.

Оба приёмника используют UDP `39824`. Патч внедряется в
`BongoCat.OSSpecific.GlobalKeyHook`: `Awake` запускает слушателя, `Update`
переносит полученные тапы в `_keysDown`, а `OnApplicationQuit` останавливает
его, поэтому тапы проходят через собственную обработку тапов игры.

> **Примечание:** `BongoSync.dll` должен быть скомпилирован против
> урезанных управляемых сборок игры. Точный вызов `mcs` и устранение проблем
> с сетью см. в `README.md` сопутствующего набора.
...
Распределение ввода также питает необязательный синхронизатор тапов по
локальной сети (`src/core/sync_net.c`). Он не зависит от путей Live2D и
оверлеев и отправляет одну UDP-датаграмму `TAP:1` при каждом нажатии клавиши
и кнопки мыши; перемещение мыши не пересылается. Синхронизатор ничего не
делает, когда отключён, и никогда не блокирует главный цикл, поскольку его
сокет неблокирующий.
